...
function parseColor(color) {
    ...
    var rgba = color.match(/^rgba\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*((\d|\B)+.*\d*)\s*\)$/i);
    if( rgba ) {
...
function RGBHEXHLS( colorString ) {
 ...
 *
 * Color Units
 * @param color String - Color designation.  Can be Alphaname, HLS, HEX or RGBA
 * @param alpha Number - {optional} Number betweeen 0 and 1 representing the opacity that your want to inforce
 * @constructor
 */
function CSSColors( color, alpha ){
        this.color = color;
        this.alpha = alpha || 0;
}
CSSColors.prototype.isColor = function() {
    if ( this.color in _colorsByName){
        return true
    }
};
CSSColors.prototype.toRGB = function(){

    var cc =  RGBHEXHLS( this.color ),
        alpha = ( this.alpha || cc[3] ) * 255;
